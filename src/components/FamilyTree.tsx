...
export default function FamilyTree({ members, onMemberClick }: FamilyTreeProps) {
  ...
  const generateTreeData = () => {
    ...
    const buildTree = (person: FamilyMember, level: number = 0, visited = new Set<string>()): TreeNode | null => {
      ...
      if (person.jenisKelamin === 'Laki-laki' && person.idPasangan) {
        ...
        const husband = members.find(m => m.idPasangan === person._id && m.jenisKelamin === 'Laki-laki');
        if (husband) {
          spouse = husband;
        }
      }
      ...
      const uniqueChildren = children
        .filter((child, index, self) => self.findIndex(c => c._id === child._id) === index)
        .sort((a, b) => {
      ...
      const childNodes = uniqueChildren.map(child => {
        if (child.jenisKelamin === 'Laki-laki') {
          ...
          return {
            id: child._id,
            member: child,
            spouse: child.idPasangan ? memberMap.get(child.idPasangan) : undefined,
    ...
    const rootMembers = members.filter(member => {
      const hasParentInDataset = members.some(p => 
        p._id !== member._id && member.idAyah === p._id
      );
  ...
  const renderMemberCard = (member: FamilyMember, isSpouse: boolean = false) => {
    return (
      <div
        onClick={() => onMemberClick(member)}
        className="flex flex-col items-center cursor-pointer group relative"
      >
        <div className={`w-20 h-20 rounded-full border-4 flex items-center justify-center text-white font-bold text-sm shadow-lg transition-all duration-200 ${
  ...
  if (treeData.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-lg p-12 text-center border border-gray-200">
        ...
        <h3 className="text-xl font-semibold text-gray-900 mb-3">Belum ada data keluarga</h3>
        <p className="text-gray-600 max-w-md mx-auto">
          Tambahkan anggota keluarga laki-laki untuk memulai pohon silsilah garis ayah
        </p>
      </div>
    );
  }
